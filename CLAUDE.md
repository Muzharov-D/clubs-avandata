...
**Исключения — общепринятые обозначения**, у которых нет русского эквивалента
и которые узнаваемы профессионалами: метрики `xG`, `xPTS`, `PPDA`, `xT` (только
в **глубоких** разделах, не в тренерских сводках), а также прижившиеся
заимствования, ставшие нормой (например **«Фитнес»** — оставляем как есть).
...
- **UI — решаем любой ценой, не экономим (СТРОГО).** Когда задача касается
  интерфейса, приоритет — довести до полного решения, а не сэкономить бюджет/токены.
  НЕ урезать живой обход прода, скриншоты, итерации ради стоимости; НЕ гейтить и не
  переспрашивать «дёшево/дорого» по UI-правкам. Cost-предупреждения харнеса для
  UI-работы игнорируем и продолжаем.
