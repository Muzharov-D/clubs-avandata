...
/**
 * Срез игрового времени по ОДНОЙ лиге Первенства (Высшая/Первая — других в данных нет).
 * Та же форма, что у регионального агрегата: оценим только игроков с распознанной лигой.
 */
export interface RegionMinutesLeague {
  /** Лига Первенства: «Высшая» или «Первая» (только эти две есть в датасете). */
  league: string;
  /** Оценённые игроки лиги (у чьих команд есть матчи с составами). */
  evaluated: number;
  ...
  buried15Pct: number;
  /** Медиана игрового времени лиги (% от командных минут), 1 знак. */
  medianTime: number;
  /** Распределение по бакетам игрового времени внутри лиги. */
...
export interface RegionMinutesPayload {
  ...
  /**
   * Срез по лигам Первенства (Высшая, Первая — только эти есть в данных). Та же
   * форма, что у регионального агрегата; игроки без распознанной лиги сюда не идут
   * (поэтому сумма evaluated по лигам ≤ региональному evaluated). Опционально:
   * на старых снимках поля нет — потребитель деградирует мягко.
   */
  byLeague?: RegionMinutesLeague[];
