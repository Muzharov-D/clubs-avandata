...
export interface RegionPyramidLeague {
  ...
  clubs: number;
  /** Нормализованные ключи клубов (для проверки счёта/дублей; опционально — старых снимков нет). */
  clubNames?: string[];
  /** Игроки, дедуп глобально, назначены в ВЫСШУЮ лигу появления. */
  players: number;
  /**
   * Различимые матчи лиги (по командам матча → лига команды). 0 по умолчанию
   * (на старых снимках поля нет — фронт деградирует мягко). Сумма matches по
   * лигам может НЕ совпасть с totals.matches: матчи без распознанной лиги
   * (команды нет в standings) в per-league не попадают, но входят в тотал.
   */
  matches: number;
