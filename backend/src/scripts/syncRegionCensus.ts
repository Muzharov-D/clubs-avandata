...
function normClub(n: string | null | undefined): string {
  ...
  let s = (n ?? '')
    .toLowerCase()
    .replace(/["'«»]/g, '')
    // Ведущий институциональный префикс юр.лица (ГБУ ДО / ГБОУ ДО / МБУ ДО / СПб ГБУ …) —
    // это форма учреждения, НЕ имя клуба: «ГБУ ДО СШОР Кировского района» = «СШОР Кировского
    // района» (иначе клуб двоился → лиги Вторая/Третья считали 11 вместо 10).
    .replace(/^\s*(?:спб\s+)?(?:гбу|гбоу|мбу|мбоу|мау|маоу)(?:\s+до)?(?:\s+дод)?\s+/u, '')
    ...
    .replace(/(?:^|\s)(?:фк|дфк|ооо|ао)(?=\s|$)/gu, ' ')
    .replace(/\s*20\d{2}\b.*$/, '')
    .replace(/-спб|санкт-петербург/g, '')
    // Аббревиатура округа «р-на»/«р-н» → «района» (ФФСПб даёт оба вида: «СШ Петроградского
    // р-на - Динамо» = «...района - Динамо»). ДО дефис→пробела (иначе «р-на» разорвётся).
    // Кириллический \b в JS — no-op, поэтому якорим токен краем/пробелом.
    .replace(/(^|\s)р-?на(?=\s|$)/g, '$1района')
    .replace(/(^|\s)р-?н(?=\s|$)/g, '$1района')
  ...
  // Явные алиасы «один клуб — два написания», которые общие правила НЕ сольют безопасно:
  // бланкетно резать СШ/СШОР НЕЛЬЗЯ («СШОР Зенит» ≠ «ФК Зенит» — РАЗНЫЕ клубы, оба в Высшей).
  // «СШОР Сестрорецк» = «Сестрорецк» (один клуб, ФФСПб даёт оба написания по когортам).
  const ALIAS: Record<string, string> = { 'сшор сестрорецк': 'сестрорецк' };
...
function assertNormClub(): void {
  const cases: Array<[string, string]> = [
    ...
    // «ДФК» (детский футбольный клуб) — тоже тип-токен: «ДФК Зенит 84» = «Зенит-84» (один
    // клуб в Третьей лиге → иначе считалось 11 вместо 10).
    ['ДФК Зенит 84', 'зенит'],
    ['Зенит-84', 'зенит'],
    ...
    // Институциональный префикс юр.лица срезаем: «ГБУ ДО СШОР Кировского района» = «СШОР
    // Кировского района» (в FFSPB по возрастам встречаются оба написания → клуб двоился).
    ['ГБУ ДО СШОР Кировского района', 'сшор кировского района'],
    ['СШОР Кировского района', 'сшор кировского района'],
    // СШ/СШОР НЕ схлопываем с базовым клубом — разные школы.
    ['СШОР Зенит', 'сшор зенит'],
    // Аббревиатура округа «р-на» = «района» (не расщеплять клуб).
    ['СШ Петроградского р-на - Динамо', 'сш петроградского района динамо'],
    ['СШ Петроградского района - Динамо', 'сш петроградского района динамо'],
    // «СШОР Сестрорецк» = «Сестрорецк» (явный алиас одного клуба), но СШОР Зенит ≠ Зенит.
    ['СШОР Сестрорецк', 'сестрорецк'],
