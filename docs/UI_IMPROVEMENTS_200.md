...
## ✅ Статус реализации (на 2026-05-31)

Реализовано **8 батчей**, все смержены в `main` через зелёный CI (typecheck + build) и
доехали до прода. Полностью закрыты **все 5 P0** и весь объём P1; из P2 закрыта
бо́льшая часть, остаток — точечная мелочь (адаптив <480px на отдельных экранах,
единичные hover-нюансы).

| Батч | Область | Коммит | Что вошло |
|---|---|---|---|
| 1 | Фундамент + P0 | `82fc00f` | маппинг ошибок логина (нет «405»), фикс белого autofill, токены контраста/моушна, a11y шапки |
| 2 | Дашборд | `436fc79` | убран невидимый `clip:text`, контраст delta/countdown, SVG вместо эмодзи, нейтральные accent |
| 3 | Матч | `c248158` | контраст, фон sticky-навигации, tabular-nums, адаптив таймлайна |
| 4 | Игрок + рейтинги | `b01a6b1` | контраст пилюль/строк, моушн |
| 5 | Календарь + Тренировки | `0bd373d` | focus-visible (P0), моушн |
| 6 | Общие компоненты | `a5e25b4` | ARIA-модалка, SVG-EmptyState, Tabs fallback |
| 7 | Админка + лендинг | `59bcfe9`,`f9d8ce3` | моушн, типографика hero, видимость статов |
| 8 | Polish-sweep | — | Toast/SpeedZones/Beeswarm `prefers-reduced-motion` + `hover:hover`; PushOptIn ease-out + scale(0.97) |

**Сквозные паттерны, разнесённые по всем батчам:** единый `--ease-out`, `:active scale`,
`prefers-reduced-motion`, `@media (hover:hover)` против залипания на тач, замена `transition:all`
на конкретные свойства, фоллбэки для CSS-переменных, токены вместо хардкода цвета.

Остаток P2 (необязательный, можно добивать точечно) помечен в списке ниже приоритетом **P2**.
