...
**F2/F3 — все «Скоро» реализованы (8/8 модулей рабочие):**
- `83dcf41` — Эпик 6 Возрастной эффект (RAE: кварталы рождения + перекос по клубам)
- `0d93b3e` — Эпик 5 Талант-пул (рейтинг по индексу эффективности, гейт согласия для имён)
- `c896984` — Эпик 6 Развитие (минуты молодых + активная глубина)
- `9725f74` — Эпик 7 Бенчмаркинг (KPI-сравнение клубов + экспорт CSV)

Эндпоинты (все read-only, через `withFederation`): `GET /api/v1/federation/{overview,clubs,competitions,data-quality,age-effect,talent,development,benchmark}`.
...
## F2/F3 — реализованы (8/8 модулей навигации рабочие)

Все ранее «Скоро»-модули добиты: талант-пул (рейтинг + гейт согласия), возрастной
эффект (RAE), продуктивность клубов, бенчмаркинг + экспорт CSV. Остаются осознанные
упрощения (не блокеры):

- **Дедуп игрока между клубами** не сделан — игрок за 2 клуба может попасть в
  талант-пул дважды (редко на практике). Полноценный дедуп по `externalIds` — далее.
- **Профиль игрока** (PizzaChart/перцентили) отдельной страницей не построен —
  талант-пул показывает рейтинг списком (имя по согласию).
- **Нормировка** в талант-пуле — абсолютный средний рейтинг, не перцентиль против
  позиционного пула региона (упрощение; методика — в `architecture.md`/CIES-референсе).
- **RAE** — по региону и клубам; разрез по возрастам можно добавить.
